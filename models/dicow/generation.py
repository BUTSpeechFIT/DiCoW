...
class DiCoWGenerationMixin(WhisperForConditionalGeneration):
    def _prepare_encoder_decoder_kwargs_for_generation(
            self, inputs_tensor: torch.Tensor, model_kwargs, model_input_name, generation_config=None, **kwargs
    ) -> Dict[str, Any]:
        if generation_config is not None:
            generation_config.output_hidden_states = True
        else:
            kwargs.get('generation_config', getattr(self, 'generation_config', None)).output_hidden_states = True
        
        # Explicitly force output_hidden_states in model_kwargs before passing to super
        model_kwargs["output_hidden_states"] = True

        # set it explicitly on model config for safety
        self.config.output_hidden_states = True

        import inspect
        sig = inspect.signature(super()._prepare_encoder_decoder_kwargs_for_generation)
    ...
    def _retrieve_init_tokens(self, input_features, generation_config, config, batch_size=1, num_segment_frames=1, kwargs=None, **extra_kwargs):
    ...
    def _retrieve_logit_processors(self, generation_config, logits_processor, begin_index, is_shortform, num_beams=1,
                                   device=None, **kwargs):
